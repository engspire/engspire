...
type IIntakeCardProps = {
  ...
  hideClassDays?: boolean;
  hideClassTime?: boolean;
  hideFees?: boolean;
  showJoin?: boolean;
};

export default function IntakeDetails({ data, showJoin, hideStartDate, hideDuration, hideClassDays, hideClassTime, hideFees }: IIntakeCardProps) {
  const { userId } = useAuth();

  return (
    <>
      {showJoin && data.startDate && <h2 className="card-title mb-4">Join the {data.status}</h2>}
      {data.startDate && !hideStartDate &&
        <div className="mb-2">
